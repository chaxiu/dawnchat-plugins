...
import {
  ...
  getMobileAssistantIdentityHandle,
...
function resolveLocalPluginId() {
  return getMobileAssistantIdentityHandle()?.assistantInstanceId || "mobile-ai-assistant";
}
...
function createCapabilityInvokeExecutionContext(
  ...
  return {
    requestId: `mobile_assistant_${Date.now()}_${Math.random().toString(16).slice(2)}`,
...
export async function invokeLocalMobileAssistantFunction(
  functionName: string,
  payload: Record<string, unknown>,
  options: Record<string, unknown> = {}
) {
