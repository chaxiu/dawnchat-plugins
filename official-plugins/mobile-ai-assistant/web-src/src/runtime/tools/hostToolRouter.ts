...
import { getExposedRuntimeCapabilityRegistrations } from "./mobileAssistantHostToolDefinitions";
import {
  ensureRuntimeEventForwardingInstalled,
  invokeLocalMobileAssistantFunction,
  toRecord,
} from "./mobileAssistantHostToolInvocation";

export { listMobileAssistantToolDefinitions } from "./mobileAssistantHostToolDefinitions";

export function createMobileAssistantHostToolRouter() {
  ensureRuntimeEventForwardingInstalled();
  ...
  const invokeDirectLocalTool = async (request: {
    ...
    invokeLocalMobileAssistantFunction(request.functionName, request.payload, request.options || {});
  ...
  const invokeDawnchatUiCapability = async (request: {
    ...
    return await invokeLocalMobileAssistantFunction(parsed.functionName, parsed.payload, parsed.options);
  };
  ...
  const invokeDawnchatUiAlias = async (
    ...
    return await invokeLocalMobileAssistantFunction(functionName, rawPayload, options);
  };
