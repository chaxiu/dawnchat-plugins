import type { MobileAssistantIdentity } from "../../runtime/assistantIdentity";

const CHAT_DEBUG_LOG_LIMIT = 200;
const CHAT_DEBUG_LOG_STORAGE_KEY_PREFIX = "dawnchat.mobile-ai-assistant.debug.chat.v1::";
...
declare global {
  interface Window {
    __DAWNCHAT_MOBILE_ASSISTANT_DEBUG__?: {
...
function toStorageKey(identity: MobileAssistantIdentity): string {
...
export function appendChatDebugLog(
  identity: MobileAssistantIdentity,
  event: string,
  data: Record<string, unknown>,
  ...
  if (typeof console !== "undefined") {
    const prefix = `[mobile-assistant/chat][${record.level}] ${record.event}`;
...
export function installChatDebugWindowHandle(identity: MobileAssistantIdentity) {
  if (typeof window === "undefined") {
    return;
  }
  const storageKey = toStorageKey(identity);
  window.__DAWNCHAT_MOBILE_ASSISTANT_DEBUG__ = {
