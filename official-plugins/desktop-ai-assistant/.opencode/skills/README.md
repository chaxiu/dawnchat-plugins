...
- Rich display request:
  - `assistant-intent-router` -> `assistant-runtime-diagnose` -> `assistant-python-sidecar-check` -> `assistant-capability-discovery` -> `assistant-rich-display-execution` -> `assistant-session-narration` -> `assistant-delivery-report`
...
- Regression check:
  - `assistant-runtime-diagnose` -> `assistant-runtime-recover` -> `assistant-evolution-verify` -> `assistant-delivery-report`
