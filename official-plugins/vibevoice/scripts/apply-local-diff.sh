...
LOCAL_DIR="${ROOT_DIR}/src/local_overrides/vibevoice"
TARGET_SUBDIR="vibevoice"

cd "${SUBMODULE_DIR}"

if git remote get-url origin | grep -q '^git@github.com:'; then
  git remote set-url origin https://github.com/chaxiu/VibeVoice.git
  git remote set-url --push origin https://github.com/chaxiu/VibeVoice.git
fi
