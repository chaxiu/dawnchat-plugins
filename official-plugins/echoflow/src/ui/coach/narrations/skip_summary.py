...
async def render_skip_summary_view(
    *,
    course: "Course",
    ...
    input_hash = narrator.compute_input_hash(
        {
            ...
            "kind": "skip_summary",
            "lang": str(lang),
            "prompt_version": "skip_summary_v1",
    ...
    cached = course_db.find_cached_narration(course_id=str(course.id), kind="skip_summary", lang=str(lang), input_hash=input_hash)
    cached_text = str((cached or {}).get("content_text") or "").strip()
    cached_audio = (cached or {}).get("tts_audio_path")

    meta = f"{range_start + 1}–{range_end + 1}" if range_end >= range_start else ""
    title = _t("skip_summary") if _t("skip_summary") != "skip_summary" else "跳过总结"
    ...
    async def _generate() -> tuple[str, Optional[str], Optional[Path]]:
        ...
        _, result = await narrator.generate_skip_summary(
            course_id=str(course.id),
            lang=str(lang),
            range_start_idx=int(range_start),
            range_end_idx=int(range_end),
        ...
        audio_path = narration_audio_path(base_dir=data_dir, course_id=str(course.id), kind="skip_summary", input_hash=result.input_hash)
        ...
        cached_row = course_db.find_cached_narration(course_id=str(course.id), kind="skip_summary", lang=str(lang), input_hash=str(result.input_hash))
