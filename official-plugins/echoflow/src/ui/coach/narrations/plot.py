...
async def render_plot_view(
    *,
    ...
    cached = course_db.find_cached_narration(course_id=str(course.id), kind="plot", lang=str(narration_lang), input_hash=input_hash)
    ...
    title = _t("plot") if _t("plot") != "plot" else "剧情"
    ...
    async def _generate() -> tuple[str, Optional[str], Optional[Path]]:
        ...
        _, result = await narrator.generate_plot(
            course_id=str(course.id),
            lang=str(narration_lang),
            range_start_idx=int(range_start),
            range_end_idx=int(range_end),
        ...
        audio_path = narration_audio_path(base_dir=data_dir, course_id=str(course.id), kind="plot", input_hash=result.input_hash)
        ...
        cached_row = course_db.find_cached_narration(course_id=str(course.id), kind="plot", lang=str(narration_lang), input_hash=str(result.input_hash))
