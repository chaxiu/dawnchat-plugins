<template>
  <template v-for="(msg, index) in processedMessages" :key="msg.id">
    <component v-if="resolveRenderer(msg.type)" :is="resolveRenderer(msg.type)" :message="msg" />
    <UserMessage
    ...
    <AckMessage
      ...
      :content="msg.content"
      :is-latest="index === processedMessages.length - 1"
    ...
    />
    <UnknownMessage v-else :content="resolveUnknownContent(msg)" />
  </template>
</template>
...
const thoughtIcon = computed(() => props.icons?.thought)
const planIcon = computed(() => props.icons?.plan)
const processedMessages = computed(() => (props.transform ? props.transform(props.messages) : props.messages))

const resolveRenderer = (type: string) => props.renderers?.[type]
...
const resolveToolResultDescription = (msg: UIMessage) =>
  (msg.payload?.result_description as string) || msg.result || (msg.payload?.result as string)
